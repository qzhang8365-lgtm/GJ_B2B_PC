 ...
 * - sibling-overlap（Badge 角标）：`.gj-badge-dot`/`.gj-badge-count` 通过
 *   `position:absolute` 故意叠在宿主图标/头像右上角（例如导航栏铃铛图标上的
 *   未读红点），这是 Badge 组件按 rules.md 的既定实现方式，不是缺陷；出现在
 *   报告里时先看两个元素是不是「Badge 角标 × 它所依附的图标/头像」，是的话
 *   直接跳过。
 * - icon-text-color-diff-needs-check（误配对）：脚本按 DOM 顺序找"图标+相邻
 *   文字"配对，遇到 Badge/Tag 等角标和旁边不相关的图标或用户名文字被当成一对
