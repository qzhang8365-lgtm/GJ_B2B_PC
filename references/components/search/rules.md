...
- `gj-search-icon` 支持图标库 SVG `<img>` 与无内容 `<span style="--gj-icon:url(...)" aria-hidden="true">` 两种载体：`img` 保持透明背景并直接显示资源，`span` 才使用 `currentColor` mask。禁止把 mask 背景规则直接施加到 `<img>`，否则会形成图标后的色块。
