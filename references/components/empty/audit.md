...
正式 Empty PNG 资源四角为不透明纯白色，并非透明底。为避免在灰色场景卡片上出现突兀的白色矩形，本轮不修改已审计插图资源，而是将整个语义场景卡片及其 Empty 舞台统一为 `Background/Container` 白色，再由外层 `Background/Background` 灰色承载面区分卡片边界。这样插图白底与组件容器自然融合，也保留了场景之间的层级。
