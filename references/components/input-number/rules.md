...
- 模式包括基础输入、左右加减、固定单位/币种、右侧步进、单值 Slider、Range Slider。Small/Medium/Large 为 100×24、112×32、133×40px，圆角 4/6/8px；Addon（步进按钮）宽度等于档位高度。单位/币种后缀条（`gj-number-input-unit-label`）不再固定等于档位高度：单字符或单个符号（如「￥」「%」「元」）渲染为与档位高度一致的正方形，两个及以上字符（如「万元」）改为按文字内容宽度自适应，左右各留 8px 内边距，不换行。
